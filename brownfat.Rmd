...
```{r}
fit_final_w_bf_inter <- lm(log(Total_vol) ~ `7D_Temp` + Cancer_Status + `7D_Temp`:Ext_Temp - 1, data = training_data_w_bf)
summary(fit_final_w_bf_inter)
extractAIC(fit_final_w_bf_inter)
```
...
First, we will see the accuracy on initial classification problem.
```{r}
# for testing data.
predicted_classes <- ifelse(predict(fit_final, testing_data) >= 0.5, 1, 0)
actual_classes <- testing_data$BrownFat

paste("testing accuracy:", sum(predicted_classes == actual_classes) / nrow(testing_data))

# for training data.
predicted_classes <- ifelse(predict(fit_final, training_data) >= 0.5, 1, 0)
actual_classes <- training_data$BrownFat

paste("training accuracy:", sum(predicted_classes == actual_classes) / nrow(training_data))
```
We observe a 92% accuracy on the testing set, and 93% accuracy on the training set, overall pretty good, and minimal overfitting.

Now we proceed to estimate the exact vol. of those w/ some level of brown fat.

```{r}
# for testing dataset
predicted_classes <- ifelse(predict(fit_final, testing_data) >= 0.5, 1, 0)

predicted_vols <- rep(0, times=nrow(testing_data))
for (i in 1:nrow(testing_data)) {
  if (predicted_classes[i] == 1) {
    predicted_vols[i] <- exp(predict(fit_final_w_bf_inter, testing_data[i, ])) 
  }
}

actual_vols <- testing_data$Total_vol

mspe <- mean((predicted_vols - actual_vols)^2)
mspe

# for training data set
predicted_classes <- ifelse(predict(fit_final, training_data) >= 0.5, 1, 0)

predicted_vols <- rep(0, times=nrow(training_data))
for (i in 1:nrow(training_data)) {
  if (predicted_classes[i] == 1) {
    predicted_vols[i] <- exp(predict(fit_final_w_bf_inter, training_data[i, ])) 
  }
}

actual_vols <- training_data$Total_vol
