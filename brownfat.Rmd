...
Recall `1M_Temp` and `Ext_Temp` are highly correlated as well as the p-value testing for regression coefficient associated w/ `1M_Temp` is larger than $\alpha = 0.05$, we will drop the covariate `1M_Temp`. Thus our final model after step-wise regression is as follows:

```{r}
fit_final <- lm(BrownFat ~ Age + Ext_Temp + Diabetes + Sex + 
                  Weigth + Season + Duration_Sunshine,
                data=training_data)
summary(fit_final)
```
