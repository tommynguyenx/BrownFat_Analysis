...
grid.arrange(g3, g4, g6, g7, g8, g9, g10, g11, g12, g13, g14, g15, g16, g19, g20, g21,ncol = 4, nrow = 4)
```
...
p2 <- ggplot(brownfat_cleaned_data_v2, aes(x = factor(Sex), y = Total_vol)) + 
  geom_boxplot() 
  #labs(title = "sex vs total volume: box plot")

#Graphing Age vs Total_volume
p3 <- ggplot(brownfat_cleaned_data_v2, aes(Age, Total_vol)) +
  geom_point(color = "black", size = 2) +
  geom_smooth(method = "lm", se = FALSE, colour = "red")
...
# Create overlapping plot
p4 <- ggplot(brownfat_long, aes(x = Values, y = Total_vol, color = Weight_types)) +
  geom_point(alpha = 0.6, size = 2) +  # Scatter points with transparency
  geom_smooth(method = "lm", se = FALSE) +  # Linear trend lines
  theme_minimal() +
  labs(x = "Weights",
...
#Graph Diabetes vs Total Volume
p5 <- ggplot(brownfat_cleaned_data_v2, aes(x = factor(Diabetes), y = Total_vol)) + 
  geom_boxplot() 
  #labs(title = "Diabetes vs total volume: box plot")

# Relationship between Glycemia and total volume.
p6 <- ggplot(brownfat_cleaned_data_v2, aes(Glycemy, Total_vol)) +
  geom_point(color = "black", size = 2) +
  geom_smooth(method = "lm", se = FALSE, colour = "red")

# Graph of relationship of cancer status vs total volume:
p7 <- ggplot(brownfat_cleaned_data_v2, aes(x = factor(Cancer_Status), y = Total_vol)) + 
  geom_boxplot()  
  #labs(title = "Cancer Status vs total volume: box plot")

# Graph of relationship of cancer type vs total volume:
p8 <- ggplot(brownfat_cleaned_data_v2, aes(x = factor(Cancer_Type), y = Total_vol)) + 
  geom_boxplot() 
  #labs(title = "Cancer type vs total volume: box plot")

# Graph of relationship of Sex vs Weight:
p9 <- ggplot(brownfat_cleaned_data_v2, aes(x = factor(Sex), y = Weigth)) + 
  geom_boxplot()
  #labs(title = "Sex vs Weight: box plot")

# Graph of relationship of Sex vs Glycemia:
p10 <- ggplot(brownfat_cleaned_data_v2, aes(x = factor(Sex), y = LBW)) + 
  geom_boxplot()
  #labs(title = "Sex vs LBW: box plot")

# Graph of relationship of Cancer Status vs Age:
p11 <- ggplot(brownfat_cleaned_data_v2, aes(x = factor(Cancer_Status), y = Age)) + 
  geom_boxplot()
  #labs(title = "Sex vs Cancer Status: box plot")

# Relationship between Age and Size
p12 <- ggplot(brownfat_cleaned_data_v2, aes(Age, Size)) +
  geom_point(color = "black", size = 2) +
  geom_smooth(method = "lm", se = FALSE, colour = "red")


grid.arrange(p2, p3, p4, p5, p6, p7, p8, p9, p10, p11, p12, nrow = 4, ncol = 3)
